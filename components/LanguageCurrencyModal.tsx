...
export function LanguageCurrencyModal({ 
  ...
  return (
    <div className="relative" ref={modalRef}>
      ...
      {isOpen && (
        <div className="absolute top-full mt-4 ltr:right-0 rtl:left-0 w-[90vw] max-w-[600px] sm:w-[600px] z-[100] drop-shadow-2xl" dir={currentLocale === "ar" ? "rtl" : "ltr"}>
          <div className="bg-[#FCFAEE] rounded-3xl w-full max-h-[85vh] overflow-hidden flex flex-col shadow-2xl relative text-[#1a2b3c] border border-black/5">
            {/* Arrow pointing up */}
            <div className="absolute -top-2 ltr:right-10 rtl:left-10 w-4 h-4 bg-[#FCFAEE] rotate-45 border-l border-t border-black/5"></div>
            
            <div className="flex-shrink-0 bg-[#FCFAEE] z-10 flex items-center justify-between px-6 py-5 border-b border-[#1a2b3c]/10 relative">
            ...
            <div className="flex-shrink-0 bg-[#FCFAEE] p-5 border-t border-[#1a2b3c]/10 flex gap-4 mt-auto relative z-10">
              <button 
                onClick={handleSave}
                className="flex-1 bg-[#d0a755] hover:bg-[#b89040] text-[#1a2b3c] font-black py-3.5 rounded-xl transition-all shadow-sm"
              >
                {t.save}
              </button>
              <button 
                onClick={() => setIsOpen(false)}
                className="flex-1 bg-white border border-[#1a2b3c]/10 hover:bg-gray-50 text-[#1a2b3c] font-bold py-3.5 rounded-xl transition-all"
              >
                {t.cancel}
              </button>
            </div>
