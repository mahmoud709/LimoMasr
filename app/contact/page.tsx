...
import { siteText, ui } from "@/lib/i18n";
import type { Locale } from "@/lib/types";
import { cookies } from "next/headers";

type ContactSearchParams = {
  __locale?: string | string[];
  lang?: string | string[];
};
...
export default async function ContactPage({
  ...
  const searchParamsResolved: ContactSearchParams =
    await (searchParams ?? Promise.resolve<ContactSearchParams>({}));

  const settings = await getSiteSettings();

  const cookieStore = await cookies();

  // Get locale from URL first, then cookie, then fallback to Arabic
  const localeParam =
    searchParamsResolved.__locale ?? searchParamsResolved.lang;
  ...
  // Make sure locale is always a supported locale
  ...
  return (
    <PublicLayout settings={settings} locale={locale}>
      <main
        className="w-full relative z-10 bg-[#F9F8F6]"
          ...
          <div className="grid gap-8 lg:grid-cols-2 animate-reveal-2">
            ...
            <div className="bg-[#1a2b3c] rounded-[2rem] p-8 md:p-10 text-white flex flex-col justify-center gap-8 shadow-2xl relative overflow-hidden border border-[#d0a755]/20 h-full text-start">
              <div
                ...
                style={{
                  backgroundImage:
                    "radial-gradient(circle at top right, #d0a755 0%, transparent 50%)",
                }}
              ></div>
              ...
              <div className="relative z-10 flex gap-6">
                ...
                <div className="flex flex-col gap-2">
                  <span className="text-xs font-black tracking-[0.2em] text-[#d0a755] uppercase">
                    {t.contactPage.hq}
                  </span>
              ...
              <div className="relative z-10 flex flex-col md:flex-row gap-6 items-start md:items-center justify-between group">
                <div className="flex gap-6">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <a
                      href="mailto:info@limo-egypt.com"
                      dir="ltr"
                      className="font-bold text-left text-2xl font-outfit text-white tracking-widest hover:text-[#d0a755] transition-colors"
                    >
                      info@limo-egypt.com
                    </a>
                  </div>
              ...
              <div className="relative z-10 flex flex-col md:flex-row gap-6 items-start md:items-center justify-between group">
                <div className="flex gap-6">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <p
                      dir="ltr"
                      className="font-bold text-left text-2xl font-outfit text-white tracking-widest"
                    >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="w-full md:w-auto flex items-center justify-center gap-3 bg-white/10 border border-white/20 text-white px-6 py-3.5 rounded-xl font-bold hover:bg-[#d0a755] hover:text-[#1a2b3c] hover:border-[#d0a755] hover:shadow-[0_10px_20px_rgba(208,167,85,0.2)] hover:-translate-y-1 transition-all"
                >
              ...
              <div className="relative z-10 flex flex-col items-center justify-center gap-4">
                ...
                <div className="flex flex-wrap justify-center gap-3 md:gap-4">
                  {[
                    {
                      icon: (
                        <FaYoutube className="w-5 h-5 md:w-6 md:h-6 group-hover:scale-110 transition-transform" />
                      ),
                      url: settings.socialLinks?.youtube || "#",
                    },
                    {
                      icon: (
                        <FaLinkedinIn className="w-5 h-5 md:w-6 md:h-6 group-hover:scale-110 transition-transform" />
                      ),
                      url: settings.socialLinks?.linkedin || "#",
                    },
                    {
                      icon: (
                        <FaTiktok className="w-5 h-5 md:w-6 md:h-6 group-hover:scale-110 transition-transform" />
                      ),
                      url: settings.socialLinks?.tiktok || "#",
                    },
                    {
                      icon: (
                    ...
                    {
                      icon: (
                        <FaFacebookF className="w-5 h-5 md:w-6 md:h-6 group-hover:scale-110 transition-transform" />
                      ),
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-white hover:bg-[#d0a755] hover:border-[#d0a755] hover:text-[#1a2b3c] hover:-translate-y-1 transition-all shadow-inner group"
                    >
                      {social.icon}
                    </a>
                  ))}
